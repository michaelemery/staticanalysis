...
Tests are assessed based on the number of true positive and false negative results generated by the checker. Test programs will generally incorporate only a single deliberate safety concern, however multiple false positives may be detected at additional points where the concern can cascade. Only one point needs to be detected for the result to be considered sound (at best). There is always at least one deliberate error, therefore the absence of a true positive implies a false negative. Any number of false positives will result in an "imprecise" rating (at best). A result is considered a false positive where safe code is identified as a concern. Where a checker correctly identifies an unrelated concern, the result is ignored. 
